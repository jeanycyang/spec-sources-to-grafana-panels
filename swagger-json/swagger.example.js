// EXAMPLE FROM: https://petstore.swagger.io/v2/swagger.json
...
module.exports = () => ({
  ...
  paths: {
    ...
    '/user/login': {
      get: {
        ...
        responses: { 200: { description: 'successful operation', schema: { type: 'string' }, headers: { 'X-Rate-Limit': { type: 'integer', format: 'int32', description: 'calls per hour allowed by the user' }, 'X-Expires-After': { type: 'string', format: 'date-time', description: 'date in UTC when token expires' } } }, 400: { description: 'Invalid username/password supplied' } },
      },
    },
    '/user/logout': {
      get: {
        tags: ['user'], summary: 'Logs out current logged in user session', description: '', operationId: 'logoutUser', produces: ['application/xml', 'application/json'], parameters: [], responses: { default: { description: 'successful operation' } },
      },
    },
    '/user/{username}': {
      get: {
  ...
  securityDefinitions: {
    petstore_auth: {
      type: 'oauth2', authorizationUrl: 'https://petstore.swagger.io/oauth/authorize', flow: 'implicit', scopes: { 'write:pets': 'modify pets in your account', 'read:pets': 'read your pets' },
    },
  ...
  definitions: {
    Order: {
      type: 'object',
      properties: {
        id: { type: 'integer', format: 'int64' }, petId: { type: 'integer', format: 'int64' }, quantity: { type: 'integer', format: 'int32' }, shipDate: { type: 'string', format: 'date-time' }, status: { type: 'string', description: 'Order Status', enum: ['placed', 'approved', 'delivered'] }, complete: { type: 'boolean', default: false },
      },
      xml: { name: 'Order' },
    },
    User: {
    ...
    Pet: {
      ...
      properties: {
        id: { type: 'integer', format: 'int64' }, category: { $ref: '#/definitions/Category' }, name: { type: 'string', example: 'doggie' }, photoUrls: { type: 'array', xml: { name: 'photoUrl', wrapped: true }, items: { type: 'string' } }, tags: { type: 'array', xml: { name: 'tag', wrapped: true }, items: { $ref: '#/definitions/Tag' } }, status: { type: 'string', description: 'pet status in the store', enum: ['available', 'pending', 'sold'] },
      },
      xml: { name: 'Pet' },
    },
